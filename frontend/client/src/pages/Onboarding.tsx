...
export default function Onboarding() {
  ...
  if (authStep === "profile") return <ProfileStep firstName={firstName} setFirstName={setFirstName} lastName={lastName} setLastName={setLastName} email={email} setEmail={setEmail} dob={dob} setDob={setDob} details={details} setDetails={setDetails} terms={terms} setTerms={setTerms} saveError={profileSaveError} saving={savingProfile} onBack={() => setAuthStep("connect")} onContinue={async () => {
    setSavingProfile(true);
    setProfileSaveError("");
    const localProfile = { firstName, lastName, email, learningProfile: learningProfile() };
    localStorage.setItem("learnloop-profile", JSON.stringify(localProfile));
    try {
      await saveProfile();
    } catch (error) {
      // A profile sync must not make the core onboarding flow unusable offline.
      setProfileSaveError("Your profile is saved on this device and will sync when the service is available.");
    } finally {
